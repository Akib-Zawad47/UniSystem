...
<div class="container mt-4 mb-5">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <table class="table table-sm">
                        <thead>
                            <tr><th>ID</th><th>Name</th><th>G-Suite</th><th>Action</th></tr>
                        </thead>
                        <tbody>
                    ...
                    <?php if($students->num_rows == 0) echo "<small class='text-muted'>No pending students.</small>"; ?>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <?php if($teachers->num_rows == 0) echo "<small class='text-muted'>No pending teachers.</small>"; ?>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <nav>
                <ul class="pagination pagination-sm justify-content-center">
                    ...
                    <li class="page-item disabled"><span class="page-link">Page <?php echo $page_pending; ?></span></li>
            ...
            <?php else: ?>
                <p class="text-muted">No pending resources.</p>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <td>
                        <form method="post" onsubmit="return confirm('Permanently delete this resource?');">
                            <input type="hidden" name="r_id" value="<?php echo $row['r_id']; ?>">
            ...
            <nav>
                <ul class="pagination pagination-sm justify-content-center">
                    ...
                    <li class="page-item disabled"><span class="page-link">Page <?php echo $page_approved; ?></span></li>
            ...
            <?php else: ?>
                <p class="text-muted">No approved resources.</p>
            <?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="row">
            ...
            <div class="card p-3">
                ...
                <form method="post">
                    ...
                    <div class="mb-2">
                        ...
                        <select name="c_selector" id="c_select" class="form-select" required>
                            <option value="">-- Select --</option>
                            <?php 
                ...
                <script>
                    document.querySelector('form').addEventListener('submit', function() {
                        var val = document.getElementById('c_select').value;
                        if(val) {
                            var parts = val.split('|');
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Course Assignments List</div>
                <table class="table table-sm">
    ...
    <?php if($view == 'directory'): 
        ...
        $search_id_val = "";
        $search_sql = "";
        if(isset($_POST['search_directory_id']) && !empty($_POST['search_directory_id'])) {
            $search_id_val = $_POST['search_directory_id'];
            if($dir_type == 'student') {
                $search_sql = " AND s_id = '$search_id_val' ";
            } else {
                $search_sql = " AND t_id = '$search_id_val' ";
            }
        }
        ...
        if ($dir_type == 'student') {
            $total_sql = "SELECT COUNT(*) as total FROM student WHERE account_status='active' $search_sql";
            $data_sql = "SELECT s_id as id, name, g_suit, department, S_Contact as phone FROM student WHERE account_status='active' $search_sql LIMIT $limit OFFSET $offset";
        } else {
            $total_sql = "SELECT COUNT(*) as total FROM teacher WHERE account_status='active' $search_sql";
            $data_sql = "SELECT t_id as id, name, g_suit, department, T_Contact as phone FROM teacher WHERE account_status='active' $search_sql LIMIT $limit OFFSET $offset";
        }
    ...
    <div class="card p-3 bg-light mb-3">
        <form method="post" class="d-flex gap-2">
            <input type="number" name="search_directory_id" class="form-control" placeholder="Search by ID" value="<?php echo $search_id_val; ?>">
            <button type="submit" class="btn btn-dark">Search</button>
            <a href="?view=directory&type=<?php echo $dir_type; ?>" class="btn btn-secondary">Reset</a>
        </form>
    </div>
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <table class="table table-striped">
                <thead><tr><th>ID</th><th>Name</th><th>Email (G-Suite)</th><th>Phone</th><th>Dept</th><th>Action</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" onsubmit="return confirm('Permanently remove this user? This will delete all their data.');">
